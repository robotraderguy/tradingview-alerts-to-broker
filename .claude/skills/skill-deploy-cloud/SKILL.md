...
**Signing in to the host WITH GitHub is not the same as granting it repo
access.** These are two separate things and only the first is obvious: the
sign-in is an OAuth login, while reading code needs the host's **GitHub App**
installed on the account that owns the repo. A user who logged in with GitHub
will reasonably say they are already connected, and the host's settings page
will show no "connect" button to press, because for login purposes it is
connected. The App can still be absent.

So when the host cannot read the repo, don't re-check the login and don't
re-run it. Have the user open their GitHub **installed apps** page
(`github.com/settings/installations`) while signed in as the account that owns
the repo, and read what it says:

- The host is **not listed** — the App was never installed. Install it.
- The host **is listed** — open it and check *repository access*. An App scoped
  to selected repositories does not cover a repo created after it was granted,
  which is the common case when the repo was made minutes earlier.

**Identities are the usual root cause, so establish them before creating
anything.** Confirm which account the host CLI is signed in as, which workspace
is active, and which account owns the repo. Three identities that each look
right individually will still fail together, and the error never names them.
Reading an existing service's repo URL is a quick way to see which GitHub
account the host is actually wired to.

**After switching host accounts, the CLI may have no active workspace** and
every command fails with a permission error that looks like a broken login. Set
the active workspace before concluding anything is wrong.
...
- **"Repository URL is invalid or unfetchable" almost never means the URL is
  malformed.** The message offers the accepted formats, which sends you off
  checking a string that was right all along. Read the other half — *unfetchable*
  — and treat it as "the host cannot see this repo". Confirm the repo exists and
  its visibility from the terminal, then go after access.
- **Prove the deployed app can reach OUT, not just be reached.** A page that
  renders live broker data is the proof; a page that renders at all is not. The
  broker call leaves the host over the open internet, and a service that accepts
  alerts but cannot call the broker places no orders while looking perfectly
  healthy.
- **A sleeping free instance is not the only gap a free plan leaves.** Requests
  landing during a restart or an edge hiccup can also fail outright and then
  succeed on retry. Since the sender never retries, treat any "it 404'd once then
  worked" as a symptom of the plan rather than a fluke worth chasing.
