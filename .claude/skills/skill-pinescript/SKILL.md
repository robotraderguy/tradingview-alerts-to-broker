...
- **Warn that an alert captures the strategy's inputs when it is created.** Editing an input
  afterwards does not reach an alert that already exists — it keeps firing with the values it
  was born with. Any input the user has to fill in themselves (a token, a password, an
  account reference) therefore has to be set BEFORE the alert is made, and changing one later
  means deleting the alert and creating it again. Say this in the setup instructions, next to
  the input concerned; without it the symptom is an alert that keeps sending an empty field
  while the chart plainly shows the value filled in.
...
### Diagnosing a live alert from the receiving end

- **Count the requests before reading them.** One event producing two POSTs means two alerts
  are active on the strategy, not one misconfigured alert. A leftover alert from an earlier
  attempt keeps firing on its own settings, and no amount of fixing the good one silences it —
  it has to be deleted. Say which of the two arrived, since only one of them is worth keeping.
- **TradingView's default order-fill text is a free readout of the strategy's inputs.** It
  prints them in declaration order inside parentheses, so an empty slot between commas is a
  blank input — the fastest way to prove a token field is actually empty rather than merely
  suspected. It is useless as a payload, but as evidence it beats guessing.
