...
- **Give every inbound alert its own id, and stamp it on every line that alert
  produces.** One alert writes several entries as it progresses, and without a
  shared id nothing downstream can tell which outcome belongs to which alert
  except by assuming the lines are adjacent — an assumption that holds right up
  until two alerts overlap, and then quietly stops being true. It costs one
  field, and anything that later reads this file for a per-alert view depends on
  it.
